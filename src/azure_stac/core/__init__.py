        ...
        try:
            ...
            for root, d, files in os.walk(commands_path):
                ...
                for p in processors:
                    modname = p
                try:
                    module = SourceFileLoader(modname, os.path.join(Path(__file__).parents[1], 'commands', p)).load_module()
...
def get_default_cli():
    ...
    return StacCli(cli_name='stac',
                   commands_loader_cls=StacCommandsLoader)